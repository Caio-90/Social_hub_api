...
import createComment from "../services/commentServices/createComment"
import findCommentById from "../services/commentServices/findCommentById"
import findCommentsByPostID from "../services/commentServices/findCommentsByPostId"
...
export default class CommentController {
  ...
  public async create(request: Request, response: Response): Promise<Response> { 
    ...
    if (commentParams.content == "") {
      return response.status(400).send({message: "O campo texto do comentário não pode estar vazio"})
    }
